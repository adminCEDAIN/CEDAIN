<?php
	...
	if ($img_id): 
?>
<div class="thim-gallery source-images" id="<?php echo esc_attr($gallery_id); ?>" data-per_page="<?php echo esc_attr($per_page); ?>">
		...
		<?php
			foreach ( $img_id as $id ) {
				...
				if ( $src ):
					...
					$image .= '			<a class="media" href="'.($info->guid).'" data-title="'.($info->post_title).'" data-caption="'.esc_attr($info->post_excerpt).'">';
