...
if ( !function_exists( 'thim_post_meta' ) ) :
	...
	function thim_post_meta($show) {
		...
		if (get_post_type() == 'post' || get_post_type() == 'portfolio'){
			...
			if ( !isset( $theme_options_data['thim_single_show_date'] ) ) {
				$show_date = $theme_options_data['thim_single_show_date'] = 1;
			}
			if ( !isset( $theme_options_data['thim_single_show_author'] ) ) {
				$theme_options_data['thim_single_show_author'] = 1;
			}
			
			if ( !isset( $theme_options_data['thim_single_show_category'] ) ) {
				$theme_options_data['thim_single_show_category'] = 1;
			}

			if ( !isset( $theme_options_data['thim_single_show_comment'] ) ) {
				$theme_options_data['thim_single_show_comment'] = 1;
			}
			...
			<ul class="entry-meta">
				...
				if ( isset( $theme_options_data['thim_single_show_date'] ) && $theme_options_data['thim_single_show_date'] == 1  && $show['date'] === true ) {
					?>
					<li class="date">
						<span><?php echo get_the_date(); ?> </span>
					</li>
				...
				if ( isset( $theme_options_data['thim_single_show_author'] ) && $theme_options_data['thim_single_show_author'] == 1 && $show['author'] === true ) {
					?>
					<li class="author">
						<?php printf( ' <a href="%1$s">%2$s</a>',
							esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
				...
				if ( isset( $theme_options_data['thim_single_show_category'] ) && $theme_options_data['thim_single_show_category'] == 1 && $show['category'] === true  ) {
					?>
					<li class="categories">
						<?php
						if( get_post_type() == 'post' ) {
				...
				if ( isset( $theme_options_data['thim_single_show_comment'] ) && $theme_options_data['thim_single_show_comment'] == 1 && $show['comment'] === true  ) {
					?>
					<?php if ( !post_password_required() && ( comments_open() || '0' != get_comments_number() ) ) :
						?>
						<li class="comment-total">
...
if ( !function_exists( 'thim_related_post' ) ) :
	...
	function thim_related_post() {
		global $post;
		$theme_options_data = get_theme_mods();
		$number 	= isset($theme_options_data['thim_single_related_number']) ? $theme_options_data['thim_single_related_number'] : 3;
		$col 		= 'col-xs-'. 12/(isset($theme_options_data['thim_single_related_column']) ? $theme_options_data['thim_single_related_column'] : 3);
		$show 		= isset($theme_options_data['thim_single_related_show']) ? $theme_options_data['thim_single_related_show'] : true;
											...
											<div class="article-title-wrapper">
												<div class="article-inner">
													<div class="media">
														...
														$img_src = aq_resize( $src[0], 270, 200, true ); 
														?>
